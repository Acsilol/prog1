/*
    g++ ch15_2.cpp -o ch15_2
*/
#include "std_lib_facilities.h"
 
const int min_year = 1;
const int max_year = 150;
 
void ev_ellenorzese(int age)
{
    if (age < min_year or age >= 150) error("Nem elfogadható az év!");
}
...
istream& operator>>(istream& is, Person& p)
{
    ...
    is >> szam;
    p.set_ev(szam);
    if (szam <= min_year || szam > max_year) error("Nem elfogadható az év!");
...
try{
    ...
    cout << "Adj meg keresztnevet meg egy vezetéknevet és egy kort kétszer." << endl;
    for (int i = 0; i < 2; ++i)
    {
        Person var;
        cin >> var;
